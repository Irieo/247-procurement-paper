...
def used():
    ...
    ldf = df.loc[[f"ci_fraction_clean_used_{t}" for t in typ]].rename(
        {f"ci_fraction_clean_used_{t}": t for t in typ}
    )
    ldf.columns = ldf.columns.map(rename_scen)
    ldf.rename({"local": "PPA", "grid": "grid imports"}, inplace=True)
    ldf = (100.0 * ldf).round(1)
    ...
    plt.text(
        4,
        yl_ref - 6,
        f"Reference case: {int(round(yl_ref,0))}" + "%",
...
def ci_capacity():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "ci_capacity.pdf"), transparent=True
    )
...
def ci_generation():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "ci_generation.pdf"), transparent=True
    )
...
def system_emissions():
    ...
    plt.text(
        3.3,
        yl_ref + 0.3,
        f"Reference case, emissions {round(yl_ref,1)} [Mt]",
        horizontalalignment="left",
        bbox=dict(facecolor="w", alpha=0.5),
    )
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "system_emissions.pdf"),
        transparent=True,
    )
...
def zone_emissions():
    ...
    plt.text(
        3.5,
        yl_ref - 0.02 * yl_ref,
        f"Reference case: {round(yl_ref,1)} Mt",
        horizontalalignment="left",
        bbox=dict(facecolor="w", alpha=0.5),
    )
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "zone_emissions.pdf"),
        transparent=True,
    )
...
def ci_emisrate():
    ...
    plt.text(
        3,
        yl_ref - 0.06 * yl_ref,
        f"Reference case: {int(yl_ref)} gCO\u2082/kWh",
        horizontalalignment="left",
        bbox=dict(facecolor="w", alpha=0.5),
    )
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "ci_emisrate.pdf"), transparent=True
    )
...
def ci_cost():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "ci_cost.pdf"), transparent=True
    )
...
def ci_costandrev():
    ...
    plt.axvline(x=0.5, color="gray", linestyle="--")
    plt.text(0.6, yl_end + 1, f"net cost at 100% 24x7 CFE", horizontalalignment="left")
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "ci_costandrev.pdf"), transparent=True
    )
...
def system_capacity():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "system_capacity.pdf"),
        transparent=True,
    )
...
def system_capacity_diff():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "system_capacity_diff.pdf"),
        transparent=True,
    )
...
def total_capacity_diff():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "total_capacity_diff.pdf"),
        transparent=True,
    )
...
def objective_rel():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "system_objective_rel.pdf"),
        transparent=True,
    )
...
def objective_abs():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "system_objective_abs.pdf"),
        transparent=True,
    )
...
def system_emisrate():
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.used.replace("used.pdf", "system_emisrate.pdf"),
        transparent=True,
    )
...
if __name__ == "__main__":
    ...
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake(
            "plot_summary", palette="p1", zone="IE", year="2025", participation="10"
        )
