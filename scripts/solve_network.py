...
def limit_resexp(n, year, snakemake):
    ...
    mask = n.generators.index.str.contains("datacenter", case=False)
...
def add_ci(n, participation, year):
    ...
    if "adv_nuclear" in clean_techs:
        n.add(
            ...
            + costs.loc["adv_nuclear"]["fuel"] / costs.loc["adv_nuclear"]["efficiency"],
            p_nom_extendable=True if policy == "cfe" else False,
    ...
    if "battery" in storage_techs:
        ...
        n.add(
            ...
            capital_cost=n.stores.at[
                f"{node} battery" + "-{}".format(year), "capital_cost"
            ],
        ...
        n.add(
            ...
            efficiency=n.links.at[
                f"{node} battery charger" + "-{}".format(year), "efficiency"
            ],
            capital_cost=n.links.at[
                f"{node} battery charger" + "-{}".format(year), "capital_cost"
            ],
            p_nom_extendable=True if policy == "cfe" else False,
            lifetime=n.links.at[
                f"{node} battery charger" + "-{}".format(year), "lifetime"
            ],
        )
        ...
        n.add(
            ...
            efficiency=n.links.at[
                f"{node} battery discharger" + "-{}".format(year), "efficiency"
            ],
            marginal_cost=n.links.at[
                f"{node} battery discharger" + "-{}".format(year), "marginal_cost"
            ],
            p_nom_extendable=True if policy == "cfe" else False,
            lifetime=n.links.at[
                f"{node} battery discharger" + "-{}".format(year), "lifetime"
            ],
        )
    ...
    if "hydrogen" in storage_techs:
        ...
        n.add(
            ...
            efficiency=n.links.at[
                f"{node} H2 Electrolysis" + "-{}".format(year), "efficiency"
            ],
            capital_cost=n.links.at[
                f"{node} H2 Electrolysis" + "-{}".format(year), "capital_cost"
            ],
            p_nom_extendable=True if policy == "cfe" else False,
            lifetime=n.links.at[
                f"{node} H2 Electrolysis" + "-{}".format(year), "lifetime"
            ],
        )
        ...
        n.add(
            ...
            efficiency=n.links.at[
                f"{node} H2 Fuel Cell" + "-{}".format(year), "efficiency"
            ],
            capital_cost=n.links.at[
                f"{node} H2 Fuel Cell" + "-{}".format(year), "capital_cost"
            ],
            p_nom_extendable=True if policy == "cfe" else False,
            lifetime=n.links.at[
                f"{node} H2 Fuel Cell" + "-{}".format(year), "lifetime"
            ],
        )
...
def solve_network(n, policy, penetration, tech_palette):
    ...
    def excess_constraints(n, snakemake):  # done
        weights = n.snapshot_weightings["generators"]
        ...
        share = snakemake.config[
            "excess_share"
        ]  # max(0., penetration - 0.8) -> no sliding share
    ...
    def res_constraints(n):  # done
        ...
        n.model.add_constraints(
            lhs == penetration * total_load, name="100RES_annual_constraint"
        )

    def country_res_constraints(n):  # done
        grid_buses = n.buses.index[
            n.buses.location.isin(geoscope(zone, area)["country_nodes"])
        ]
        ...
        n.model.add_constraints(
            lhs == target * total_load, name="country_res_constraints"
        )
    ...
    def country_res_constraints_incl(n):
        ...
        ci_res_gens = n.generators.index[
            n.generators.bus.isin(ci_bus) & n.generators.carrier.isin(ci_res_techs)
        ]
    ...
    def system_res_constraints(n, snakemake):
        ...
        for ct in country_targets.keys():
            ...
            # If "incl": count corporate PPA within local zone's RES target
                ...
                if ct == zone:
                    grid_buses = n.buses.index[
                        (n.buses.index.str[:2] == ct) | (n.buses.index == f"{ci_name}")
                    ]
                else:
                    grid_buses = n.buses.index[(n.buses.index.str[:2] == ct)]
            ...
            print(
                f"country RES constraint for {ct} {target} and total load {total_load/1e6} TWh"
            )
            logger.info(
                f"country RES constraint for {ct} {target} and total load {total_load/1e6} TWh"
            )
    ...
    def system_res_constraints_necp(n, snakemake):
        ...
        for ct in country_targets.keys():
            if ct == zone:
                ...
                n.model.add_constraints(
                    lhs == (target * total_load - ci_load), name=f"{ct}_res_constraint"
                )
                ...
                target = timescope(ct, year, snakemake)["country_res_target"]
                total_load = (
                    n.loads_t.p_set[country_loads].sum(axis=1) * weights
                ).sum()
    ...
    def extra_functionality(n, snapshots):
        ...
        if snakemake.config["system_res_constraint"] == "system":
            ...
            if snakemake.config["country_res_constraint"] == "default":
                ...
                print(
                    f"'country_res_constraint' setting must be one of 'default', 'incl'. Now is {snakemake.config['country_res_constraint']}."
                )
                sys.exit()
    ...
    grid_cfe_df = pd.DataFrame(
        0.0,
        index=n.snapshots,
        columns=[f"iteration {i}" for i in range(n_iterations + 1)],
    )
...
if __name__ == "__main__":
    ...
    n = pypsa.Network(
        timescope(zone, year, snakemake)["network_file"],
        override_component_attrs=override_component_attrs(),
    )

    Nyears = 1  # years in simulation
    costs = prepare_costs(
        timescope(zone, year, snakemake)["costs_projection"],
