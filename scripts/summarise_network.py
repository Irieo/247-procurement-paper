...
def summarise_network(n, policy, tech_palette):
    ...
    p_clean = (
        n.generators_t.p[clean_gens]
        .multiply(n.snapshot_weightings["generators"], axis=0)
        .sum(axis=1)
    )
    p_storage = -n.links_t.p1[clean_dischargers].multiply(
        n.snapshot_weightings["generators"], axis=0
    ).sum(axis=1) - n.links_t.p0[clean_chargers].multiply(
        n.snapshot_weightings["generators"], axis=0
    ).sum(
        axis=1
    )
    p_demand = n.loads_t.p["datacenter load"].multiply(
        n.snapshot_weightings["generators"], axis=0
    )
    ...
    used_grid = (
        -n.links_t.p1[name + " import"].multiply(
            n.snapshot_weightings["generators"], axis=0
        )
    ) * grid_cfe
    ...
    results["ci_fraction_clean"] = (
        results["ci_clean_total"] / results["ci_demand_total"]
    )
    results["ci_fraction_clean_used_local"] = (
        results["ci_clean_used_local_total"] / results["ci_demand_total"]
    )
    results["ci_fraction_clean_used_grid"] = (
        results["ci_clean_used_grid_total"] / results["ci_demand_total"]
    )
    results["ci_fraction_clean_used"] = (
        results["ci_clean_used_total"] / results["ci_demand_total"]
    )
    results["ci_fraction_clean_excess"] = (
        results["ci_clean_excess_total"] / results["ci_demand_total"]
    )
    ...
    rest_system_buses = n.buses.index[
        ~n.buses.index.str.contains(name) & ~n.buses.index.str.contains(country)
    ]
    ...
    clean_grid_links = n.links.index[
        n.links.bus1.isin(rest_system_buses) & n.links.carrier.isin(grid_clean_techs)
    ]
    ...
    line_imp_subsetA = n.lines_t.p1.loc[:, n.lines.bus0.str.contains(country)].sum(
        axis=1
    )
    line_imp_subsetB = n.lines_t.p0.loc[:, n.lines.bus1.str.contains(country)].sum(
        axis=1
    )
    ...
    ci_emissions_t = n.links_t.p0["datacenter import"] * grid_supply_emission_rate

    results["ci_emission_rate_true"] = (
        ci_emissions_t.sum() / n.loads_t.p["datacenter load"].sum()
    )
    ...
    results["ci_emission_rate_local"] = (
        n.links_t.p0["datacenter import"] * emissions_factor_local
    ).sum() / n.loads_t.p["datacenter load"].sum()
    ...
    results["ci_emission_rate_myopic"] = (
        n.links_t.p0["datacenter import"] * emissions_factor
    ).sum() / n.loads_t.p["datacenter load"].sum()

    # 3.3: Total CO2 emissions for 24/7 participating customers
    results["ci_emissions"] = (
        ci_emissions_t * n.snapshot_weightings["generators"]
    ).sum()

    # 3.4: Total CO2 emissions in zone where 24/7 participating customers are located
    results["emissions_zone"] = (
        country_hourly_emissions * n.snapshot_weightings["generators"]
    ).sum() / 1e6
    ...
    for charger in storage_chargers:
        ...
        if name + " " + charger in n.links.index:
            results["ci_cap_" + charger.replace(" ", "_")] = (
                n.links.at[name + " " + charger, "p_nom_opt"] * temp["eff_" + charger]
            )
        else:
            results["ci_cap_" + charger.replace(" ", "_")] = 0.0
    ...
    for discharger in storage_dischargers:
        ...
        if name + " " + discharger in n.links.index:
            results["ci_cap_" + discharger.replace(" ", "_")] = (
                n.links.at[name + " " + discharger, "p_nom_opt"]
                * temp["eff_" + discharger]
            )
        else:
            results["ci_cap_" + discharger.replace(" ", "_")] = 0.0

    # Storing generation at CI node

    for tech in clean_techs:
        results["ci_generation_" + tech] = (
            n.generators_t.p[name + " " + tech]
            .multiply(n.snapshot_weightings["generators"], axis=0)
            .sum()
        )
    ...
    for discharger in storage_dischargers:
        if name + " " + discharger in n.links.index:
            results["ci_generation_" + discharger.replace(" ", "_")] = (
                -n.links_t.p1[name + " " + discharger]
                .multiply(n.snapshot_weightings["generators"], axis=0)
                .sum()
            )
        else:
            results["ci_generation_" + discharger.replace(" ", "_")] = 0.0
    ...
    system_gens = gen_expandable[
        ~gen_expandable.index.str.contains("datacenter")
    ]  # drop datacenter gens

    for gen in exp_generators:
        temp["system_optcap_" + gen] = system_gens.loc[
            system_gens.index.str.contains(f"{gen}"), "p_nom_opt"
        ].sum()
        results["system_inv_" + gen.replace(" ", "_")] = (
            temp["system_optcap_" + gen]
            - system_gens.loc[system_gens.index.str.contains(f"{gen}"), "p_nom"].sum()
        )
    ...
    for charger in exp_chargers:
        ...
        results["system_inv_" + charger.replace(" ", "_")] = (
    ...
    total_cost = 0.0
    for tech in clean_techs:
        results["ci_capital_cost_" + tech] = (
            results["ci_cap_" + tech]
            * n.generators.at[name + " " + tech, "capital_cost"]
        )
        results["ci_marginal_cost_" + tech] = (
            results["ci_generation_" + tech]
    ...
    results["ci_marginal_cost_grid"] = (
        n.links_t.p0[name + " import"]
        * n.snapshot_weightings["generators"]
        * n.buses_t.marginal_price[node]
    ).sum()
    ...
    results["ci_revenue_grid"] = (
        n.links_t.p0[name + " export"]
        * n.snapshot_weightings["generators"]
        * n.buses_t.marginal_price[node]
    ).sum()
    results["ci_average_revenue"] = (
        results["ci_revenue_grid"] / results["ci_demand_total"]
    )

    if "battery" in storage_techs:
        results["ci_capital_cost_battery_storage"] = (
        ...
        results["ci_capital_cost_battery_inverter"] = (
            n.links.at[f"{name} battery charger", "p_nom_opt"]
            * n.links.at[f"{name} battery charger", "capital_cost"]
        )
        results["ci_cost_battery_inverter"] = results[
            "ci_capital_cost_battery_inverter"
        ]
        total_cost += results["ci_cost_battery_inverter"]
    else:
        results["ci_capital_cost_battery_storage"] = 0.0
        results["ci_cost_battery_storage"] = 0.0
        results["ci_capital_cost_battery_inverter"] = 0.0
        results["ci_cost_battery_inverter"] = 0.0

    if "hydrogen" in storage_techs:
        results["ci_capital_cost_hydrogen_storage"] = (
            n.stores.at[f"{name} H2 Store", "e_nom_opt"]
            * n.stores.at[f"{name} H2 Store", "capital_cost"]
        )
        results["ci_cost_hydrogen_storage"] = results[
            "ci_capital_cost_hydrogen_storage"
        ]
        total_cost += results["ci_cost_hydrogen_storage"]

        results["ci_capital_cost_hydrogen_electrolysis"] = (
            n.links.at[f"{name} H2 Electrolysis", "p_nom_opt"]
            * n.links.at[f"{name} H2 Electrolysis", "capital_cost"]
        )
        results["ci_cost_hydrogen_electrolysis"] = results[
            "ci_capital_cost_hydrogen_electrolysis"
        ]
        total_cost += results["ci_cost_hydrogen_electrolysis"]

        results["ci_capital_cost_hydrogen_fuel_cell"] = (
            n.links.at[f"{name} H2 Fuel Cell", "p_nom_opt"]
            * n.links.at[f"{name} H2 Fuel Cell", "capital_cost"]
        )
        results["ci_cost_hydrogen_fuel_cell"] = results[
            "ci_capital_cost_hydrogen_fuel_cell"
        ]
        total_cost += results["ci_cost_hydrogen_fuel_cell"]
    else:
        results["ci_capital_cost_hydrogen_storage"] = 0.0
        results["ci_cost_hydrogen_storage"] = 0.0
        results["ci_capital_cost_hydrogen_electrolysis"] = 0.0
        results["ci_cost_hydrogen_electrolysis"] = 0.0
        results["ci_capital_cost_hydrogen_fuel_cell"] = 0.0
        results["ci_cost_hydrogen_fuel_cell"] = 0.0
    ...
    results["zone_average_marginal_price"] = n.buses_t.marginal_price[node].sum() / len(
        n.snapshots
    )
...
if __name__ == "__main__":
    ...
    grid_cfe_df = pd.read_csv(snakemake.input.grid_cfe, index_col=0, parse_dates=True)
    print(grid_cfe_df)
