...
Afterward, we delve deeper into the scenario space, altering highlighted parameters to explore various alternatives and address the following questions:
...
For interested readers, we publish an online dashboard with results for the entire scenario space, see \nameref{sec:code}.
...
\cref{fig:10-2025-IE-p1-used} shows the fraction of the fraction of hourly demand met with carbon-free electricity depending on the procurement policy that participating consumers follow.
We contrast two main cases, one where participating consumers commit to 100\% annual renewable matching policy (hereinafter -- \enquote{100\% RES}) and another case where the consumers commit to 24/7~CFE hourly matching policy (hereinafter -- \enquote{24/7~CFE}) with various \gls{cfe} targets in a range from 80\% to 100\%. 
...
In the reference case, only 54\% of demand is met with \gls{cfe} electricity.
...
\subsection{Unpacking 24/7 CFE procurement's role in grid decarbonisation}
